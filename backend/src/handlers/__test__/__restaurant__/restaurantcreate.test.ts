...
describe('/review/customer',()=>{
    it("it will create a review of a customer about a specific restaturant",async()=>{
        const result=await axios.post("http://localhost:3000/app/v1/restaurant/restaurantreview",{
    ...
    expect(result.data.restaurant.name).toBe("test restaurant");
    expect(result.data.restaurant.location).toBe("test address");
    },15000)
});